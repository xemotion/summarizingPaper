...
def mainPage() : 
    ...
    with st.form("select_month") : 
        ...
        if submitted:
            ...
            with bottom_menu[2]:
                batch_size = st.selectbox("Page Size", options=[5, 25, 100])
            ...
            with bottom_menu[1]:
                total_pages = (
                        int(len(monthly_data) / batch_size) if int(len(monthly_data) / batch_size) > 0 else 1
                )
            ...
            for i in range(batch_size):
                curr_pos = i + current_page
                paper_col1, paper_col2 =  st.columns([1,10])
                with paper_col1: 
                    
                    pdf_url = data.iloc[curr_pos]['pdf_url']                             
                    pdf_link = f'[PDF]({pdf_url})'
                    st.markdown(pdf_link, unsafe_allow_html=True)
                with paper_col2: 
                    
                    text = monthly_data.iloc[curr_pos]['summary'] 
                    st.write(str(count) + ". TITLE: " + monthly_data.iloc[curr_pos]['title']) 
